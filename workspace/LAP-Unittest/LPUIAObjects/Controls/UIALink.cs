...
{
    [Guid("F56F3F94-388B-45BC-8D73-991EFCF3DDE8")]
    public interface IUIALink
    {
        [DispId(1)]
        ...
        // Common methods
        [Description("Click UIAImage, MouseKey=1 is left button,\"2\" is right button, default is \"1\"")]
        void Click(int x = 0, int y = 0, int mousekey = 1);

        [Description("Double Click UIAImage, MouseKey=1 is left button,\"2\" is right button, default is \"1\"")]
        void DBClick(int x = 0, int y = 0, int mousekey = 1);

        string Text { [Description("Get the UIAImage test")] get; }

        [Description("Check whether UIAImage exists, default value of times is \"0\"")]
        ...
        [Description("Drag UIAImage")]
        void Drag(int x = -1, int y = -1);

        [Description("Drop UIAImage")]
        void Drop(int x = -1, int y = -1);

        string Name { [Description("Get the UI name of UIAImage")]get; }

        int Hwnd { [Description("Get the handle of UIAImage")] get; }

        int X { [Description("Get absolute location of X of UIAImage")] get; }

        int Y { [Description("Get absolute location of Y of UIAImage")] get; }

        int Height { [Description("Get Height of UIAImage")] get; }

        int Width { [Description("Get Width of UIAImage")] get; }

        bool Enabled { [Description("Get Enabled of UIAImage")] get; }

        bool Focused { [Description("Get Focused of UIAImage")] get; }

        string HelpText { [Description("Get Help Text of UIAImage")] get; }

        string LabeledText { [Description("Get Labeled text of UIAImage")] get; }

        string Value { [Description("Get Value of UIAImage")] get; }

        int ProcessID { [Description("Get ProcessID of UIAImage")] get; }

        [Description("Press one or more keystrokes, value can be \"Hi Ants Automation,{DELETE}\"")]
        void PressKeys(string keys);

        // For UIALink
        string URL { [Description("Get URL of UIALink")] get; }
    }

    [ComVisible(true)]
    [ClassInterface(ClassInterfaceType.None)]
    [Guid("08E6660D-70B7-44D2-9582-3A241DF3F11B")]
    [ProgId("LeanPro.UIALink")]
    [TypeLibType((short)2)]
    [ComSourceInterfaces(typeof(IUIALink))]
    [Description("Automation UIA Link")]
    public class UIALink : UIAContainerBase,IUIALink
    {
        public UIALink(UIACondition condition)
            : base(condition)
        { }
        ...
        public string URL
        {
            get
            {
                ...
                return (string)this._condition.AutomationElement.GetCurrentPropertyValue(ValuePattern.ValueProperty);                
            }
        }
    }
