...
{
    [Guid("D417ACBD-9682-4516-9B5A-46762FE7E7F9")]
    public interface IUIAButton
    {
        [DispId(1)]
        ...
        [Description("Click UIAButton, MouseKey=1 is left button,\"2\" is right button, default is \"1\"")]
        void Click(int x = 0, int y = 0, int mousekey=1);

        [Description("Double Click UIAButton, MouseKey=1 is left button,\"2\" is right button, default is \"1\"")]
        void DBClick(int x = 0, int y = 0, int mousekey = 1);

        string Text { [Description("Get the UIAButton test")] get; }

        [Description("Check whether UIAButton exists, default value of times is \"0\"")]
        ...
        [Description("Drag UIAButton")]
        void Drag(int x = -1, int y = -1);

        [Description("Drop UIAButton")]
        void Drop(int x = -1, int y = -1);

        string Name { [Description("Get the UI name of UIAButton")]get; }
        
        int Hwnd { [Description("Get the handle of UIAButton")] get; }
        
        int X { [Description("Get absolute location of X of UIAButton")] get; }
        
        int Y { [Description("Get absolute location of Y of UIAButton")] get; }
        
        int Height { [Description("Get Height of UIAButton")] get; }

        int Width { [Description("Get Width of UIAButton")] get; }

        bool Enabled { [Description("Get Enabled of UIAButton")] get; }
        
        bool Focused { [Description("Get Focused of UIAButton")] get; }

        string HelpText { [Description("Get Help Text of UIAButton")] get; }

        string LabeledText { [Description("Get Labeled text of UIAButton")] get; }

        string Value { [Description("Get Value of UIAButton")] get; }

        int ProcessID { [Description("Get ProcessID of UIAButton")] get; }
    ...
    [Guid("8806C87A-5C03-4065-935E-07AEA8DA5838")]
    [ProgId("LeanPro.UIAButton")]
    [TypeLibType((short)2)]    
    [Description("Automation UIA Button")]
    public class UIAButton : UIAContainerBase, IUIAButton
    {
        public UIAButton(UIACondition condition)
            : base(condition)
        { }
